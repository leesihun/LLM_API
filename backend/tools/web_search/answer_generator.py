...
class AnswerGenerator:
    ...
    async def generate(
        ...
        try:
            ...
            answer = await self._generate_with_llm_async(system_prompt, user_prompt)
    ...
    async def _generate_with_llm_async(self, system_prompt: str, user_prompt: str) -> str:
        """Generate answer using LLM (async version)."""
        ...
        # Call LLM asynchronously (with timeout if configured)
        config = {}
        if self.timeout:
        ...
        response = await loop.run_in_executor(
            None,
            lambda: llm.invoke(messages, config=config) if config else llm.invoke(messages)
        )
    ...
    def _generate_with_llm(self, system_prompt: str, user_prompt: str) -> str:
        """Generate answer using LLM (sync version - kept for backward compatibility)."""
        from langchain_core.messages import SystemMessage, HumanMessage
    ...
    async def generate_batch(self, queries_and_results: List[Tuple[str, List[SearchResult]]], user_location: Optional[str] = None) -> List[Tuple[str, List[str]]]:
        ...
        for query, results in queries_and_results:
            answer, sources = await self.generate(query, results, user_location)
